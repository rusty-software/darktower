...
(deftest init-bazaar-test
  (testing "initializes with basics and things the player is missing"
    ...
    (let [{:keys [warrior food beast scout healer]} (init-bazaar (assoc player :beast true))]
      (is (every? #(not (nil? %)) [warrior food scout healer]))
      (is (nil? beast)))
    (let [{:keys [warrior food beast scout healer]} (init-bazaar (assoc player :beast true :scout true))]
      (is (every? #(not (nil? %)) [warrior food healer]))
      (is (nil? beast))
      (is (nil? scout)))
    (let [{:keys [warrior food beast scout healer]} (init-bazaar (assoc player :beast true :scout true :healer true))]
